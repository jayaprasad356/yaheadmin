...
class custom_functions
{
    ...
    public function process_shiprocket($order_id)
    {
        ...
        $sr_length = 0;
        $sr_breadth = 0;
        $sql = 'SELECT p.pickup_location,GROUP_CONCAT(DISTINCT oi.id SEPARATOR ",") AS item_ids FROM `order_items` oi left JOIN product_variant pv on oi.product_variant_id=pv.id left join products p on p.id=pv.product_id WHERE oi.order_id=' . $order_id . ' GROUP BY oi.seller_id,p.pickup_location,p.pickup_postcode';
        $this->db->sql($sql);
        $res = $this->db->getResult();
        $x = 0;
        if (isset($res[0]) && !empty($res[0])) {
            foreach ($res as $row) {
                $item_ids = $res[0]['item_ids'];
                $item_ids = explode(",", $item_ids);
                for ($i = 0; $i < count($item_ids); $i++) {
                    $sql = 'select p.standard_shipping,p.pickup_location,p.pickup_postcode,p.weight,p.length,p.breadth,p.height,pv.price,pv.discounted_price from product_variant pv left join products p on p.id=pv.product_id left join order_items oi on oi.product_variant_id=pv.id where oi.id=' . $item_ids[$i];
                    $this->db->sql($sql);
                    $pr_info = $this->db->getResult();
                    $pickup_location_id = $pr_info[0]['pickup_location'];
                    $pickup_postal_code = $pr_info[0]['pickup_postcode'];
                    $token = $shiprocket->generate_token();
                    $token = $token['token'];
                    if ($pr_info[0]['standard_shipping'] == 1) {
                        $sql = "select o.pincode_id,o.area_id,oi.date_added,oi.sub_total,oi.product_name,oi.quantity,oi.discounted_price,oi.order_id,oi.user_id from order_items oi left join orders o on o.id=oi.order_id where id=" . $item_ids[$i];
                        $this->db->sql($sql);
                        $res_oi = $this->db->getResult();
                        ...
                        $area_id = $res_oi[0]['area_id'];
                        $pincode_id = $res_oi[0]['pincode_id'];
                        $data = [
                            'name' => $product_name,
                            'sku' => '00' . $x,
                            'units' => $quantity,
                        ...
                        $serviceability_data = array(
                            ...
                            'billing_pincode' => $res_user_info['billing_pincode'],
                            'cod' => 1, // change as required
                            'weight' => $pr_info[0]['weight'],
                            'length' => $pr_info[0]['length'],
                            'breadth' => $pr_info[0]['breadth'],
                            'height' => $pr_info[0]['height']
                        ...
                        if (!empty($serviceability) && isset($serviceability['status']) && $serviceability['status'] == 200) {
                            array_push($order_items_arr, $data);
                            $weight = $pr_info[0]['weight'];
                            $sr_weight += $weight;
                            $length = $pr_info[0]['length'];
                            $sr_length += $length;
                            ...
                            $height = $pr_info[0]['height'];
                            $sr_height += $height;
                            $sub_total = $res_oi[0]['sub_total'] * $res_oi[0]['quantity'];
                            $sr_subtotal += $sub_total;

                            $data = array(
                                'order_id' => $order_id,
                                'order_item_id' => $item_ids[$i],
                            ...
                            $this->db->insert('order_trackings', $data);
                            $data = array(
                                'shipping_method' => 'standard'

                            );
                            $this->db->update("order_items", $data, "id=$item_ids[$i]");
                        }
                    }
                }
                $billing_customer_name = $res_user_info['user_name'];
                $billing_city = $res_user_info['billing_city'];
                $billing_pincode = $res_user_info['billing_pincode'];
                $billing_state = $res_user_info['billing_state'];
                $billing_country = $res_user_info['billing_country'];
                $billing_email = $res_user_info['billing_email'];
                $billing_phone = $res_user_info['billing_phone'];
                $billing_address = $res_user_info['billing_address'];

                $sql = "select date_added from orders where id=" . $order_id;
                $this->db->sql($sql);
                $res_order_date = $this->db->getResult();
                $order_date = $res_order_date[0]['date_added'];

                $sql = "select pickup_location from pickup_locations where id=" . $row['pickup_location_id'];
                $this->db->sql($sql);
                $res_pickup_location = $this->db->getResult();
                // print_r($res_pickup_location);

                $data = array(
                    'order_id' => $order_id,
                    'order_date' => $order_date,
                    'pickup_location' => $res_pickup_location[0]['pickup_location'],
                ...
                if (!empty($res) && isset($res['order_id']) && !empty($res['order_id'])) {
                    ...
                    $response['error'] = false;
                    $response['message'] = 'Shiprocket order created successfully';
