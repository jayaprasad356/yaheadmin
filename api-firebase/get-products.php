...
if (isset($_POST['get_all_products']) && $_POST['get_all_products'] == 1) {
    ...
    */
    ...
    $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
    $db->sql($sql);
    $total = $db->getResult();
