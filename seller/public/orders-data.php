...
$sql = "SELECT oi.*,o.final_total as payable_total,oi.id as order_item_id,v.product_id,v.measurement_unit_id,p.cancelable_status,o.*,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.name as uname,u.country_code FROM `order_items` oi JOIN users u ON u.id=oi.user_id LEFT JOIN product_variant v ON oi.product_variant_id=v.id LEFT JOIN products p ON p.id=v.product_id JOIN orders o ON o.id=oi.order_id WHERE o.id = $ID AND p.seller_id = $seller_id";
$db->sql($sql);
$res = $db->getResult();
$items = [];
                                            ...
                                            ?>
                                                <div class="card col-md-3">
                                                    <div class="card-body">
                                                        ...
                                                        if ($is_product == 1) {
                                                            $view_product = " <a href='" . DOMAIN_URL . "seller/view-product-variants.php?id=" . $item[0] . "' class='btn btn-success btn-xs' title='View Product'><i class='fa fa-eye'></i></a>";
                                                        }
                                                        // echo $view_product;
